...
public class ReplyFrontController1 extends HttpServlet {
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		Action action = null;
		System.out.println("re_frontcontroller1");
		if (command.equals("/ReplyWriteAction1.rr")) {
			...
			action= new ReplyDelete1();
			try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				e.printStackTrace();}
		} else if (command.equals("/ReplyReplyWriteAction.re")) {
			action= new ReplyReplyWriteAction1();
			try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				e.printStackTrace();}
