...
<%
	...
	List<PlanSpotCommentBean> list = pcdao.getSpotListComment(spot, startRow, pageSize);
%>
...
<%if(list.size()==0) {
	...
	for(int i=0; i<list.size(); i++) {
		...
		<tr>
			...
			MemberBean mb = mdao.getMemberNick(pscb.getNick());
			...
			<span class="c_nick"><%=pscb.getNick() %></span><span class="c_date"><%=pscb.getDate() %></span>
			<span class="c_eval"><%if(pscb.getEval()==1) {%>좋아요!<%}%>
								<%if(pscb.getEval()==2) {%>괜찮아요.<%}%>
								<%if(pscb.getEval()==3) {%>별로에요!<%}%></span>
			<span class="c_content"><%=pscb.getContent() %></span>
			</td>
			<td>
				<%
				if(id!=null && mb.getId().equals(id) || id!=null && sessionIdMb.getAuth()==0) {
					%>
					<a href="javascript:spotCommentDelete('<%=pscb.getNum()%>', '<%=pageNum%>');">삭제</a>
					<%
				}
				%>
			</td>
	...
	<div class="page">
		...
		if(currentPage > pageBlock) {
			%>
			<a href="javascript:spotCommentList('<%=startPage-pageBlock%>');">
				<img src="./images/etc/backward_icon.png">
			</a>
			<%
		}
		for(int i=startPage; i<=endPage; i++) {
			%><a href="javascript:spotCommentList('<%=i%>');"
