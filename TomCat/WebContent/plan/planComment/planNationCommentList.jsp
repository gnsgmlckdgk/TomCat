...
<%if(list==null) {
		...
		<tr>
			...
			<td>
				...
				if(mb.getId().equals(id)) {
					%>
					<a href="javascript:nationCommentUpdate('<%=pncb.getNum()%>');">수정</a><br>
					<%
				}
				if((id!=null && mb.getId().equals(id)) || sessionIdMb.getAuth()==0) {
					%>
	...
	<div class="page">
		...
		if(currentPage > pageBlock) {
			%>
			<a href="javascript:nationCommentList('<%=startPage-pageBlock%>');">[이전]</a>
			<%
		}
		for(int i=startPage; i<=endPage; i++) {
			%><a href="javascript:nationCommentList('<%=i%>');"><%=i %></a><%
		}
		if(endPage < pageCount) {
			%><a href="javascript:nationCommentList('<%=startPage+pageBlock%>');">[다음]</a><%
		}
		%>
